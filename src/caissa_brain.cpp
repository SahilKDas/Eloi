...
BrainResponse CaissaBrain::search(Board board, SearchLimits limits,
  ...
  response.requested = BrainIdentity::caissa_1_25;
  response.selected = BrainIdentity::caissa_1_25;
  if (!impl_->ready) {
  ...
  response.search.depth = !impl_->stopped && limits.depth > 0 ? limits.depth : 0;
