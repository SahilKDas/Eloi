...
BrainResponse response;
    std::ostringstream search_telemetry;
    std::streambuf* worker_protocol = std::cout.rdbuf(
        search_telemetry.rdbuf());
    try {
      response = brain.search(*board, limits);
    } catch (...) {
      std::cout.rdbuf(worker_protocol);
      return 74;
    }
    std::cout.rdbuf(worker_protocol);
...
const auto ready = read_line(
        std::chrono::steady_clock::now() + std::chrono::seconds(5));
    if (!ready || *ready != "READY") {
      failure = "Caissa worker startup handshake failed";
      close_worker(true);
      return false;
    }
    return true;
...
BrainIdentity IsolatedCaissaBrain::identity() const noexcept {
  return BrainIdentity::caissa_1_25;
}
...
BrainResponse IsolatedCaissaBrain::search(
  ...
  BrainResponse response;
  response.requested = response.selected = BrainIdentity::caissa_1_25;
#ifndef _WIN32
