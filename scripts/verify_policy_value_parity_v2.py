#!/usr/bin/env python3
"""Verify Python/C++ EPV2 inference on deterministic Standard positions."""
...
def load_python(path: Path) -> training.Network:
    ...
    with path.open("rb") as stream:
        ...
        expected = (training.MAGIC, training.INPUTS, training.HIDDEN,
                    training.PROMOTIONS, training.MOVES)
        if values != expected:
            raise ValueError("invalid EPV2 header")
        for array in arrays:
            raw = stream.read(array.size * 4)
            if len(raw) != array.size * 4:
                raise ValueError("truncated EPV2 array")
            array[:] = np.frombuffer(raw, dtype="<f4").reshape(array.shape)
        if stream.read(1):
            raise ValueError("trailing EPV2 bytes")
    return network
